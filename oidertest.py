...
def openurl():
    #####
    try:
        # response = urllib2.urlopen('https://'+targethost+':'+targetport+'/bin
        # /querybuilder.json?1_property=legacyUid&1_property.operation=like&1_
        # property.value=%25gn%25&p.hits=selective&p.limit=-1&p.properties=jcr%3
        # apath%20legacyUid%20&path=%2fcontent%2fsites')

        # response = urllib2.urlopen('https://pcqp1s1.alsac.stjude.org:3443/bin/
        # querybuilder.json?1_property=legacyUid&1_property.operation=like&1_
        # property.value=%25gn%25&p.hits=selective&p.limit=-1&p.properties=jcr%3
        # apath%20legacyUid%20&path=%2fcontent%2fsites')

        response = urllib2.urlopen(
            'https://toybox-s1.alsac.stjude.org/chris/badoiderstring.txt')
...
for rs in hitsResult:
    path = rs['jcr:path']
    pathCheck = isIncluded(
        path,
        1,
        1000,
        "ABCDEFGHIJKLMNOPQRSTUVQXWYXabcdefghijklmnopqrstuvwxyz0123456789/,:-_")
    ...
    for vp in get_iterable(rs['legacyUid']):
        ...
        for oid in get_iterable(UIDs):
            ...
            oidCheck = isIncluded(
                ...
                1,
                40,
                "ABCDEFGHIJKLMNOPQRSTUVQXWYXabcdefghijklmnopqrstuvwxyz0123456789")
            if oidCheck == "True":
